	...
	if(ret>0){
		...
		switch(cmd_type){
			...
			case GET_MESSAGE_LIST_RSP:
			{
				...
				printf("message list count  : %d\n",common_rsp.get_message_list_rsp().message_list_size());//调用对应的size方法
				for(int i=0;i<message_count;i++){
					printf("%6d : %s\n",common_rsp.get_message_list_rsp().message_list(i).publisher_id(),common_rsp.get_message_list_rsp().message_list(i).content().c_str());
				}
			}	
			break;
			default:
...
int delay=0;
int update(struct timeval &tim){//在time_last 的基础啊上加delay
	tim.tv_usec = tim.tv_usec + delay;
	if(tim.tv_usec > 1000000){//保证微秒的值一直正确
...
int Reg10000(){//生成一万个注册用户，进行模拟 会经过数据库
	int ret=0;
...
int main(){
	...
	while(client_on){
		...
		if(time_now>time_begin){//每隔一段时间输出收发包的数据
			time_begin=time_now;
			count_down--;
			if(count_down<=0){
				phase=999;
		...
		int need_send=0;//压力模型测试。如果满足条件才进行发包
		...
		switch(phase){
			...
			case 9:
				CHECK_SEND
				ret=AddFriend(saved_user_id1,saved_user_id2);// 2 作为1的好友  2 发布消息， 使用1 的id 请求好友消息列表
				GOTO_NEXT
				break;
			case 10:
				WAIT_RSP_GO_NEXT
			...
			case 15:
                ...
                //printf("client send GetMessageList ret = %d\n", ret); 
				GOTO_NEXT
				break;
			case 16:
				WAIT_RSP_GO_NEXT
                //printf("client receive GetMessageList rsp ret = %d \n", ret); 
				break;
			case 17:
				CHECK_SEND
				ret=DelFriend(saved_user_id1,saved_user_id2);
                //printf("client send delfriend\n"); 
				GOTO_NEXT
				break;
			case 18:
				//WAIT_RSP_GO_NEXT //将会直接结束
				WAIT_RSP_GO_LOGIN  //将会继续登录
				break;
			default:
				ret=ShutDown();
				GOTO_NEXT
